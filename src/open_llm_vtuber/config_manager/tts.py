...
class VoiceVoxTTSConfig(I18nMixin):
    ...
    port: int = Field(50021, alias="port")
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        "host": Description(
            en="VoiceVox engine host address", zh="VoiceVox 引擎主机地址"
        ),
        "port": Description(
            en="VoiceVox engine port", zh="VoiceVox 引擎端口"
        ),
        "speaker_id": Description(
            en="Speaker ID (VoiceVox character/voice ID)", zh="说话人 ID（VoiceVox 角色/声音 ID）"
        ),
        "speed_scale": Description(
            en="Voice speed scale (1.0 is normal)", zh="语速缩放（1.0 为正常）"
        ),
        "pitch_scale": Description(
            en="Voice pitch adjustment (0.0 is normal)", zh="音高调整（0.0 为正常）"
        ),
        "intonation_scale": Description(
            en="Intonation strength scale (1.0 is normal)", zh="语调强度缩放（1.0 为正常）"
        ),
        "volume_scale": Description(
            en="Volume scale (1.0 is normal)", zh="音量缩放（1.0 为正常）"
        ),
    }
...
class TTSConfig(I18nMixin):
    ...
    voicevox_tts: Optional[VoiceVoxTTSConfig] = Field(
        None, alias="voicevox_tts"
    )
