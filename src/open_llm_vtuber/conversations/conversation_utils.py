...
async def handle_audio_output(
    ...
    async for audio_path, display_text, transcript, actions in output:
        ...
        # 실시간 스트리밍을 위해 audio_path가 딕셔너리인지 확인
        if isinstance(audio_path, dict) and "audio" in audio_path:
            # 이미 준비된 오디오 데이터가 있는 경우 (실시간 스트리밍)
            audio_payload = {
                "type": "audio",
                "audio": audio_path["audio"],
                "volumes": audio_path.get("volumes", []),
                "slice_length": audio_path.get("slice_length", 20),
                "display_text": display_text.to_dict() if display_text else None,
                "actions": actions.to_dict() if actions else None,
                "forwarded": False,
            }
            logger.info("실시간 오디오 데이터 전송 중...")
        else:
            # 기존 방식의 파일 기반 오디오 재생
